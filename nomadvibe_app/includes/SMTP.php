...
class SMTP
{
    ...
    protected function getSMTPConnection($host, $port = null, $timeout = 30, $options = [])
    {
        ...
        { 
            ...
            $socket_context = stream_context_create($options); set_error_handler([$this, 'errorHandler']); 
        ...
        { 
            ...
            $this->setError('Failed to connect to server', '', (string) $errno, $errstr); $this->edebug('SMTP ERROR: '.$this->error['error'].": $errstr ($errno)",self::DEBUG_CLIENT);
    ...
    protected function sendCommand($command, $commandstring, $expect)
    {
        ...
        if (preg_match('/^([\d]{3})[ -](?:([\d]\\.[\d]\\.[\d]{1,2}) )?/', $this->last_reply, $matches)) 
        {
            ...
            $code   = (int) $matches[1]; $code_ex = (count($matches) > 2 ? $matches[2] : null);
